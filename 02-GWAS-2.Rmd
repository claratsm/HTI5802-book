...
## Prequisites
To load the `bzgip` software that is pre-installed in the VM
...
```bash
md5sum chrAll.ASA.afterSampleQC.afterVariantQC.*
```
...
```bash
mkdir associationtest
cd associationtest
cp ../chrAll.ASA.afterSampleQC.afterVariantQC.* .
cp ../CAD_LDL.pheno .
cp ../practical3.manhattanPlot.R .
```
